...
sdxl_path = "/home/li/桌面/ComfyUI/models/checkpoints/sd_xl_base_1.0.safetensors"
...
def diffusion_generation(model, prompt, height, width, num_inference_steps, guidance_scale, batch_size, lora=None):
    if model == "SDXL":
        ...
        pipe = pipe.to("cuda")
        image = \
            pipe(prompt, height=height, width=width, num_inference_steps=num_inference_steps,
                 guidance_scale=guidance_scale,
                 batch_size=batch_size).images[0]
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.img_model == "SDXL":
            ...
            if genre == "建筑风格":
                st.session_state.lora = st.multiselect(
                    "建筑风格",
            ...
            if genre == "自定义训练风格":
                st.session_state.lora = st.selectbox(
                    "自定义训练风格",
                    options=["梵高", "山水画"],
                    index=0
                )
