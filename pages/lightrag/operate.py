...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        logger.info(log_message)
        st.info(
            f"已经处理 {processed_chunks} 个chunks，总共 {total_chunks} 个。抽取到 {entities_count} 个实体 + {relations_count} 个关系")
...
"""
    ...
    st.markdown(
        f"查询到 {len(entities_context)} 个实体, {len(relations_context)} 个关系, {len(text_units_context)} 个文档片段")
